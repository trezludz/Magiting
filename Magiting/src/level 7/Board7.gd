...
func _process(delta):
	if touch and !win:
		if touch.position.x >= 0 and touch.position.x <= 180 and touch.position.y >=0 and touch.position.y <= 180:
			...
			if tiles == solved:
				win = true
				$"../../Details".text = "Post-war Philippines saw an influx of imported American products under the Philippine Rehabilitation Act of 1946. Among these products are cars and canned goods. This also implies the start of neo-colonialism in the Philippines by the Americans"
				$"../../PuzzleButton".show()
				$"../../AutoComplete".hide()
