...
func _process(delta):
	if touch and !win:
		if touch.position.x >= 0 and touch.position.x <= 180 and touch.position.y >=0 and touch.position.y <= 180:
			...
			if tiles == solved:
				win = true
				$"../../Details".text = "Once the Japanese invaders siezed control of the Philippines and established a Puppet Government, they issued a new currency. This was called 'Mickey Mouse' money by Filipinos due to its value being drastically decreased because of hyperinflation."
				$"../../PuzzleButton".show()
				$"../../AutoComplete".hide()
