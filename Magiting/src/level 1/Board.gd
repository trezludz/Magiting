...
func _process(delta):
	if touch and !win:
		if touch.position.x >= 0 and touch.position.x <= 180 and touch.position.y >=0 and touch.position.y <= 180:
			...
			if tiles == solved:
				win = true
				$"../../Details".text = "The Manunggul Jar was a burial jar used by early Filipinos. This indicates that early Filipinos already had burial rites in their culture even before the Spanish Colonizers arrived."
				$"../../PuzzleButton".show()
				$"../../AutoComplete".hide()
