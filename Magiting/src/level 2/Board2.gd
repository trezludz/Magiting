...
func _process(delta):
	if touch and !win:
		if touch.position.x >= 0 and touch.position.x <= 180 and touch.position.y >=0 and touch.position.y <= 180:
			...
			if tiles == solved:
				win = true
				$"../../Details".text = "The Cedula served as a tax identification certificate. It also states a person's membership across a city or a town. Katipuneros would tear these certificates before a battle as a sign of rising up against tyranny."
				$"../../PuzzleButton".show()
				$"../../AutoComplete".hide()
