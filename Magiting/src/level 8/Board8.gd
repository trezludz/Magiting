...
func _process(delta):
	if touch and !win:
		if touch.position.x >= 0 and touch.position.x <= 180 and touch.position.y >=0 and touch.position.y <= 180:
			...
			if tiles == solved:
				win = true
				$"../../Details".text = "The First Quarter Storm in the 1970s was a series of mass protests carried out by students and activists against the Marcos Administration. Anti-American Imperialist sentiments and corruption within the government were the main reasons for these protests. Protesters were met with force by the Philippine Metropolitan Command, more commonly known as the MetroCom. Numerous students were killed and wounded during the First Quarter Storm."
				$"../../PuzzleButton".show()
				$"../../AutoComplete".hide()
