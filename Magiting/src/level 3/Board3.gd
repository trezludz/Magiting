...
func _process(delta):
	if touch and !win:
		if touch.position.x >= 0 and touch.position.x <= 180 and touch.position.y >=0 and touch.position.y <= 180:
			...
			if tiles == solved:
				win = true
				$"../../Details".text = "The Spoliarium was painted by Juan Luna in 1884. Luna was awarded the first of three gold medal prizes when this painting was exhibited in Madrid. However, it did not win the 'prize of honor' which created a prolonged controversy among art critics and artists all over Spain."
				$"../../PuzzleButton".show()
				$"../../AutoComplete".hide()
