...
func _process(delta):
	if touch and !win:
		if touch.position.x >= 0 and touch.position.x <= 180 and touch.position.y >=0 and touch.position.y <= 180:
			...
			if tiles == solved:
				win = true
				$"../../Details".text = "The Legislative Building was designed by Ralph Harrington Doane, Antiono Toledo, and Juan Arellano and was built between 1918-1926. In February 1945, the building was used by the Japanese Occupation forces as a stronghold, and was bombarded by the American forces. It was reconstructed in 1949, and it is presently the National Museum of Fine Arts."
				$"../../PuzzleButton".show()
				$"../../AutoComplete".hide()
